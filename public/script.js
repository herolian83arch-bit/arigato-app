      ...
      <div class="onomatopoeia-item" data-testid="dict-row" onclick="handleOnomatopoeiaItemClick(event, ${item.id})">
        <div class="item-header">
          ...
          <div class="item-actions" style="display:inline-flex;align-items:center;">
            ${isTTSEnabled ? `
              <button class="speak-btn" onclick="playAudioWithFallback('', '${(item.originalMain || item.main).replace(/'/g, "\\'")}', 'ja-JP')" aria-label="音声再生" style="background:none;border:none;cursor:pointer;font-size:1.2em;margin-left:12px;" data-card-control="true">
                🔊
              </button>
            ` : ''}
